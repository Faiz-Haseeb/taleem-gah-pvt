...
def display_faculty_records():
    ...
    if len(session.get('user_info_name')) != 0 and len(session.get('user_info_username')) != 0 and session.get('user_info_type') == 'Admin':
        ...
        return render_template('view_faculty_records.html', len = len(faculty_record_list), faculty_records_object = faculty_record_list, l = len(faculty_record_list[len(faculty_record_list)-1]))
...
def edit_faculty_records():
    if len(session.get('user_info_name')) != 0 and len(session.get('user_info_username')) != 0 and session.get('user_info_type') == 'Admin':
        if request.method == 'POST':
            ...
            edit_item = []
            for x in range(len(faculty_record_list[0])-1):
                edit_item.append(request.form[str(x)])


            print(faculty_record_list)

            list_subject = []
            for y in range(len(faculty_record_list[0][-1])):
                c = y + (len(faculty_record_list[0]) - 1)
                list_subject.append(request.form[str(c)])
            edit_item.append(list_subject)
            ...
            print("HELLO")
            print(len(faculty_record_list[0][-1]))
            print(int(total_subjects_object[0][0]))
            # print(len(edit_item[6]))
            print(edit_item)

            
            return render_template('edit-faculty-records.html', edit_item = edit_item, subject_count = int(total_subjects_object[0][0]), l = len(edit_item[-1]))
    else:
        return redirect(url_for('login', message = "Please login."))
